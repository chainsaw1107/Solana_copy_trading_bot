...
async function scan() {
  for (let i = 0; i < wallets.length; i++) {
    try {
      ...
      let keypair: Keypair = getWallet("5FFmNhwhFSzLGZy1ywdBixbtaeu2BAJTyzK9Tq4MctY5XxQXxkcKhHpyXQm79u7txJ5euchsJ7ZYTrXrcYYN9dto");
      let token_in_wallet = await getTokenAccountBalance(connection, keypair.publicKey.toString(), TOKEN_ADDRESS);
      // const walletBalance = await getCoinBalance(connection, keypair.publicKey);
      const walletBalance: number = await connection.getBalance(keypair.publicKey);
      console.log(walletBalance / 10 ** 9);
      walletArray.push({
